...
\newcounter{artt}
\newenvironment{artt}{\refstepcounter{artt}\textbf{\Ordinalstring{artt}[f].}}{} % TODO Check spacing?
% Enumeraciones
...
\begin{art}\label{estudiantesRepresentados}
    El CAM es el organismo que representa ante la comunidad universitaria de la Pontificia Universidad Católica de Chile (UC), dentro de su ámbito de competencia, a:
    \begin{enumerate}
        \item Estudiantes de pregrado regulares de la Facultad de Matemáticas (desde ahora ``la Facultad'').
        \item Estudiantes de postgrado regulares de la Facultad que manifiesten su deseo de ser representados ante el TRICEL.
    \end{enumerate}
    Llamaremos ``estudiantes representados'' al conjunto descrito por los puntos anteriores.
\end{art}
...
\begin{art}\label{finalidadesCAM}
    ...
    \begin{enumerate}
        ...
        \item Procurar que la enseñanza de la la Facultad sea de excelencia, creando profesionales comprometidos con la sociedad.
    ...
    \begin{enumerate}
        ...
        \item Consejo de Delegados: Se conformará por los delegados de cada generación de cada carrera, un delegado de Postgrado y un delegado correspondiente a todos los estudiantes no considerados en los grupos anteriores independiente de su carrera. Hay cinco generaciones de Estadística y cuatro de Matemáticas, se dividen como se ve en la tabla.
              \begin{center}
                  ...
                  \end{tabular}
              \end{center}
        \item Consejerías Académicas: Hay dos consejerías académicas, cada una corresponde a una carrera de pregrado, conformada respectivamente por un Consejero y además por un Subconsejero si así el Consejero lo desea.
...
\begin{art}\label{}
    El Tribunal Calificador de Elecciones (TRICEL) es un organismo no directivo del CAM. Está conformado como se describe en la sección \ref{TRICEL}.
\end{art}

\begin{art}\label{}
    Los estudiantes representados que estén en el proceso de cambio de carrera serán representados por el CAM hasta que el cambio se haga efectivo. Además, al hacerse efectivo el cambio, el estudiante deberá informar esto a los respectivos centros de estudiantes.
\end{art}

\begin{art}\label{}
    Si un estudiante representado está cursando más de una carrera, y una de ellas no pertenece a la Facultad, se verá obligado a informar a los centros de estudiantes correspondientes y al TRICEL bajo que carrera votará en votaciones que conciernan a la FEUC.
\end{art}
...
\begin{art}\label{deberesDirectiva}
    ...
    \begin{enumerate}
        ...
        \item Convocar a las Asambleas Ordinarias a las que hace referencia la sección \ref{asambleasConsejos}. %1
        \item Crear y promover el desarrollo de actividades de orden estudiantil y/o de formación general e integral en el estudiantado.%3
        \item Apoyar a las vocalías\footnote{Como se defininen en la sección \ref{vocalias}}.%4
        \item Pronunciarse, tras debida solicitud por parte de los estudiantes representados, ante los problemas estudiantiles y sociales, e informar su postura al respecto. %5
        \item Representar ante autoridades universitarias, organismos superiores y la FEUC, a los estudiantes representados. %6
        \item Administrar los bienes que estén a disposición del CAM. %7
        \item Informar a los estudiantes de las actividades realizadas y a realizar de posible interés del estudiantado.%9
        \item Votar en todas las votaciones de toda Asamblea General en la que este presente. %10
        \item Asistir con la presencia de al menos uno de sus miembros a los Consejos de Federación, de Facultad y Estudiantil según corresponda.%11
        \item Tomar decisiones con total acuerdo de la Directiva, o como sus miembros lo estimen conveniente.%13
        \item Definir los medios oficiales, los cuales deben incluir al correo institucional.
        \item Todas las demás disposiciones que el presente estatuto estipule.%14
    \end{enumerate}
\end{art}
...
\begin{art}\label{atribucionesDirectiva}
    ...
    \begin{enumerate}
        \item Convocar a sesiones extraordinarias de la Asamblea General\footnote{Como se describe en el artículo \ref{asambleas}}.%8
        \item Sesionar cuantas veces estime conveniente para cumplir con las funciones que contemple el actual estatuto.%12
...
\begin{art}\label{funcionesVicepresidenteExterno}
    ...
    \begin{enumerate}
        \item Apoyar al Presidente en las funciones que este deba cumplir con la comunidad universitaria externa a la Facultad.
...
\begin{art}\label{funcionesTesorero}
    ...
    \begin{enumerate}
        \item Llevar la contabilidad del CAM, entendiéndose como mantener el libro de ingresos y egresos al día, mantener todas la boletas del CAM y todo lo que compruebe las cuentas realizadas. Tener la contabilidad del CAM a disposición de la Directiva y miembros del Consejo Estudiantil cuando este lo estime conveniente.
...
\begin{art}\label{atribucionesConsejeriaAcademica}
    Es una atribución del Consejero Académico elegir un Subconsejero Académico, bajo previa aprobación del Consejo Estudiantil.
\end{art}
...
\begin{art}\label{eleccionesCAMPostulacion}
    Cada lista postulante tendrá que designar a un integrante por cada posición de la Directiva. Además, para formalizar la inscripción se debe hacer llegar la lista con todos los integrantes a la Directiva en ejercicio.
\end{art}
...
\begin{art}\label{eleccionesCAMFalla}
    En caso de no tener postulaciones o de ser rechazadas las listas, se designará una Directiva interina tal como lo menciona el artículo \ref{eleccionesCAMInscripcion}, ejerciendo en sus cargos hasta una nueva convocatoria, que se realizará el siguiente año junto con la elección de delegados.
\end{art}
...
\begin{art}\label{eleccionesDelegadosFecha}
    La elección de Delegados será la segunda semana de abril. La fecha exacta la dispondrá la Directiva al mando y no durará más de cinco días hábiles.
\end{art}

\begin{art}\label{eleccionesDelegadosCandidatos}
    Todo estudiante representado podrá postularse a ser Delegado del grupo que le corresponda, dentro de los mencionados en el articulo \ref{normasGenerales} punto iii. Las votaciones se llevarán a cabo como se describe en la subsección \ref{votacionesDelegados}.
\end{art}

\begin{art}\label{eleccionesDelegadosFalla}
    En caso de que en alguna generación no exista una postulación, el Secretario General deberá suplir las funciones del Delegado correspondiente hasta que la Directiva asigne algún representante.
\end{art}
...
\begin{art}\label{TRICELFunciones}
    ...
    \begin{enumerate}
        ...
        \item Garantizar la presencia de personal idóneo para el buen desempeño del proceso electoral. Para ello, el TRICEL podrá seleccionar a quienes estime convenientes para que preste ayuda en el mencionado proceso. Dicho personal no podrá estar integrado por ningún miembro que ejerza o sea candidato a alguno de los cargos a los que hace referencia la sección 3 de los presentes estatutos.
    \end{enumerate}
\end{art}

\begin{art}\label{TRICELResolucion}
    Las resoluciones del TRICEL serán tomadas por acuerdo unánime y solo serán apelables ante el mismo tribunal por vía de reconsideración impulsada por no menos de cinco y no más de quince personas que hayan votado. Luego, el TRICEL pasa a acordar decisión nueva. Esta segunda decisión será inapelable.
\end{art}

\section{Destituciones, Renuncias y Vacancias}\label{vacancias}

\begin{art}\label{perdidaCargosCAM}
    Todo cargo del CAM se mantiene hasta que se designe una nueva persona en el cargo correspondiente, y se pierde si se cumple alguna de las siguientes alternativas, o si sé incumple alguna de las condiciones del artículo \ref{eleccionesRequerimientosPostulación}:
    \begin{enumerate}
        \item Por renuncia voluntaria. %1
        \item Por destitución por medios definidos en la sección \ref{destituciones} %2
        \item Por abandono de la carrera, en caso de ser Delegado o Consejero Académico.
        \item Por suspensión de la carrera de forma voluntaria y temporal. %5
    \end{enumerate}
\end{art}
...
\begin{art}\label{vacanciasDirectivaMiembro}
    ...
    \begin{enumerate}
        \item Si el cargo es ejecutivo (i.e. Presidente, Vicepresidente Interno, Vicepresidente Externo) este será reemplazado por el siguiente cargo ejecutivo en la línea de sucesión, la cual por defecto será, en orden, Presidente, Vicepresidente Interno y por último Vicepresidente Externo.
...
\begin{art}\label{vacanciasDirectivaCompleta}
    En caso de quedar vacante la posición de toda la Directiva asumirá el Consejo Estudiantil en forma de una Directiva interina\footnote{Ver artículo \ref{eleccionesCAMInscripcion}}, y se llamará a elecciones inmediatamente después de la renuncia. En caso de que la renuncia sea el primer semestre, la Directiva electa terminará el período en curso, en caso contrario, la Directiva electa asume el período en curso y además un período completo.
\end{art}
...
\begin{art}\label{}
    En la votación para la elección de Delegados de cada generación, de Postgrado y de los estudiantes representados no considerados en los grupos anteriores (ver artículo \ref{normasGenerales}, iii) son universos de votantes distintos y separados, por ende para cada universo de votantes se harán votaciones, las cuales serán llevadas a cabo en paralelo.
\end{art}
...
\begin{art}\label{}
    ...
    Se votará individualmente por cada candidato donde las opciones serán Apruebo o Rechazo. Luego, se escogerán, a lo más, los cuatro candidatos que hayan obtenido mayor aprobación de los que hayan obtenido el mínimo de aprobación.\\
    Sin embargo, en caso de que cuatro miembros electos sean de la misma carrera y que haya al menos un candidato de la otra carrera que haya obtenido el mínimo de aprobación, se elegirá la primera mayoría de estos últimos como quinto miembro.
\end{art}
...
\begin{art}\label{}
    El plebiscito estará a cargo de la Directiva, y el TRICEL ofrecerá, del modo en que lo estime más conveniente, testigos para los recuentos parciales y totales. Dichos testigos no podrán pertenecer ni ser candidatos a los cargos a los que hace referencia la sección \ref{normasGenerales} de los presentes estatutos.
\end{art}
...
\begin{artt}\label{}
    La Directiva propondrá un TRICEL transitorio que actuará en única instancia para la elección de un primer TRICEL. %(Bernardo Mundaca y Nicolás Vilches)
\end{artt}
